<?php
/*
 * Se encarga de mover los Recibos al directorio de timbrado
 */
include '/volume1/web/Services/Pila_factura_proveedor.php';
class Monitor_factura_proveedor {
...
//            $directorio="/volume1/Inbox_Nomina_TXT";
            ...
            foreach ($escaneo as $valor)
            {
                ...
                {
                    array_push($this->initial_files, $valor);/* Ingreso de datos en arreglo */
                }
       ...
       if(count($getNewFiles)>0)
       {
           $estado=TRUE;                                                   
            ...
            if(count($this->stack_xml_pdf)>0)
            {
                ...
                $pila=new Pila_factura_proveedor();
...
//            printf("\n Eliminado = $valor");
        ...
        $archivo_estado=fopen("/usr/CFDI/Factura_Proveedor/eliminados.txt", "r+");    
...
$monitor_nomina_xml=new Monitor_factura_proveedor("/usr/CFDI/Nomina_XML/contador.txt", "/usr/CFDI/Factura_Proveedor/estado.txt", "/volume1/Inbox_Factura_proveedor");
